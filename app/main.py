...
def load_benchmarking_results():
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Error decoding the test database JSON.")
...
def get_average_results_within_window(start_time: str, end_time: str):
    ...
    results = load_benchmarking_results()
    filtered_results = [
        r for r in results if start_dt <= r.timestamp <= end_dt
    ]
    ...
    avg_results = {
        "avg_token_count": sum(r.token_count for r in filtered_results) / len(filtered_results),
        "avg_time_to_first_token": sum(r.time_to_first_token for r in filtered_results) / len(filtered_results),
        "avg_time_per_output_token": sum(r.time_per_output_token for r in filtered_results) / len(filtered_results),
        "avg_total_generation_time": sum(r.total_generation_time for r in filtered_results) / len(filtered_results),
    }
    return avg_results
